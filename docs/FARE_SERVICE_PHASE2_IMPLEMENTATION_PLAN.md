...
## Interim pricing authority

Until an approved transfer-of-authority gate activates the Fare Service, the existing Fiji Tour Transfers booking workflow is the **Interim Pricing Authority** for the narrowly approved Stage 1 transfer scope. Its human owner is the Fiji Tour Transfers commercial pricing owner, accountable to James. Executable behavior currently found in `ftt-booking-site/src/app.js` is implementation evidence for that workflow, not a second policy authority; `workers/chat-widget/worker.js`, Lagi, ComeToFiji, WhatsApp, and operator tools may not maintain or improvise competing prices. Outside the approved scope, consumers must return pricing unavailable or route to authorized manual review.

Every interim price or manual adjustment must record the source/version or approved price-list revision, normalized journey and service time, FJD line items and total, actor, reason, effective time, quote/reference ID, and change approval in the Audit Evidence Service. Changes require a change record, commercial-owner review, fixture parity evidence against approved current cases, and James's applicable release approval; corrections create a new version rather than rewriting historical quotes. Scope is limited to currently approved Fiji Tour Transfers products and pricing.

Authority transfers only at milestone M8 after James records an explicit transfer decision against a GitHub-visible Fare Service artifact and rule-set version. Required evidence includes approved canonical policy, Booking API publication, versioned migrations, security and contract tests, fixture/parity results, shadow and canary evidence, consumer cutover inventory, rollback rehearsal, and confirmation that interim and duplicate execution is disabled. Until that decision, the Fare Service is a candidate and the Interim Pricing Authority remains the sole authority for its approved scope.

## Required product coverage

The model must explicitly support Nadi Airport→resort, resort→Nadi Airport, Nausori Airport routes, resort→resort, airport→activity, and activity→resort journeys; private and shared transfers; vehicle classes; night surcharges; extra stops; child-seat and service extras; tour add-ons; partner fares; discounts; promotions; and effective dates/version history. Settlement is in FJD. Later AUD, NZD, and USD values are display-only conversions supplied by the Currency Rate Service with source, version, observed-at, effective-at and expiry; quotes and settlement retain FJD. Without an unexpired authoritative rate, conversion is omitted rather than estimated.
...
A rule has immutable rule ID/version; draft/review/approval state; effective-from/to; canonical origin, destination, direction and journey type; private/shared product; vehicle class and capacity; base calculation in integer FJD cents; typed night, stop, child-seat/service, and tour adjustments; partner eligibility; discount/promotion decision references and stacking; tax/fee and rounding policy; precedence; public label/reason codes; and approval evidence. The Promotion Service owns campaigns, eligibility, budgets, and redemptions; Fare consumes its decision and owns only the deterministic monetary effect. Unknown or over-capacity inputs return `NotPriced`, never an inferred mapping or combination.
...
9. **M8 transfer and sole authority:** after all evidence is approved, James records the exact effective time, Fare Service artifact/rule-set, scope, rollback target, and explicit transfer from the Interim Pricing Authority; disable interim and duplicate execution for that scope, prove inventory clean, and retain approved rollback compatibility.
