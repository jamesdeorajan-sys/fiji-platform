...
**CORRECTION (2026-09-21):** my earlier posts said "108 genuine / 43 tests". One row carried a `TEST-` reference and had not been caught by the name/phone rules. With that rule added: **151 rows − 44 rows excluded by the current test rules = 107 rows *not excluded by current test rules*** (raw). Six of those 107 are suspected tests awaiting James's classification (see below), so "genuine" is not yet established. All figures below are the corrected ones.
...
**Scope of the totals.** The table below covers the **whole history, 24 Jul → 20 Sep** — not September. The September-only view is in `data/sept_1_20_fiji_totals.csv` (complete Fiji days 1–20 Sep): after the D1 retry rule **82 rows / FJ$11,376.29 quoted** (on-site 43 / 5,972.00; FijiDash-Nadi 8 / 1,501.96; FijiDash-other 27 / 3,350.84; no-ref 4 / 551.49; Nadi combined 51 / 7,473.96); raw 86 rows / FJ$11,933.29. Before September (24 Jul → 31 Aug): 20 rows / FJ$2,697.94 (82 + 20 = 102; 11,376.29 + 2,697.94 = 14,074.23). Sensitivity variants for the same window are in that file (Sept 1–20 range 74–86 rows, FJ$10,489.61–11,933.29).
...
| **Total (whole history, 24 Jul → 20 Sep)** | | **107** (not excluded by current test rules) | **102** | **14,074.23** (raw, no dedup: 14,716.52) |
...
2. *Retry rule D1 (primary series):* same phone + destination zone + vehicle within 15 minutes → later row dropped: **5 rows** (1 no-ref, 1 FijiDash-other, 3 on-site). Four of the five also share the full trip key. The fifth (on-site, 10.2 min apart, same phone and email) has a **different pickup date** and so may be a genuine second booking rather than a correction — a judgement call; keeping it would make on-site 44 and the retry-rule total 103. Each exclusion and flag is listed, without identifiers, in `data/dedup_exclusions_and_flags.csv` (rule, action, storefront, gap minutes, same phone/email/trip key/date/zone/vehicle, Fiji day, quoted FJD); the booking ids are in a private sheet held by James.
