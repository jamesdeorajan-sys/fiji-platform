...
function updatePricing() {
  ...
  const km  = roadKm(p.lat, p.lng, d.lat, d.lng);
  ...
  state.priceSource = priced.source;  // 'published' | 'estimate'
...
// ═══════════════════════════════════════════════════════════════════════════
// CEO P0 REVENUE RECOVERY (2026-09-07, preview only) — real server-side save.
// Reuses the SAME nadi-dispatch-api Worker and D1 `bookings` table FijiDash
// already writes to (createBookingRecord()/handleGuestBookingCreate() in
// nadi-marketplace/worker/worker.js) - zero backend changes, zero new
// database, zero risk to FijiDash's own booking path. This site becomes a
// second, independent frontend against an already-hardened backend, exactly
// as instructed ("reuse the proven FijiDash booking API/core... do not
// create a separate conflicting booking database").
//
// Scope boundary, explicit and conservative: only pickup === 'NAN' (an
// arriving guest, this site's actual positioning - "airport search
// specialist") with a FIXED (non-custom) destination gets the new
// server-save path. CUSTOM_PICKUP and CUSTOM_DEST keep today's existing
// WhatsApp-only behaviour completely unchanged - this snapshot of the site
// has no geocoding/quote infrastructure to safely resolve a custom
// address's zone (that's Milestone 11/12 machinery FijiDash has and this
// frozen build never received), and inventing a zone would risk a wrong
// price or a wrong driver dispatch. Not a regression: identical to what
// happens today for every guest right now.
const NADI_API_BASE = 'https://api.nadiairporttransfers.com';

// Same real, unambiguous zone list/alias the Worker's `zones` table
// actually contains - copied verbatim from book.fijidash.com/app.js, not
// re-derived, so this can never drift into inventing a zone name the
// backend won't recognise.
...
// Resolves the destination_zone POST /bookings needs, for a FIXED
// destination only. Returns null if the option has no data-area (shouldn't
// happen for a real dropdown option), or 'NEEDS_LOOKUP' for the small
// number of areas with no 1:1 zone match - callers treat NEEDS_LOOKUP as
// "not eligible for server-save this round", same conservative rule as
// CUSTOM_PICKUP/CUSTOM_DEST above, never a guessed zone.
...
// Includes body reading in the deadline: a connected but stalled response
// must not leave the guest locked on Saving. A timeout is an unknown outcome.
...
async function submitNadiBooking(ref, destZone) {
  ...
  const payload = {
    ...
    distance_km: state.distanceKm || null,
    payment_method: 'cash',
...
// Same non-blocking, fire-and-forget escalation pattern as FijiDash's
// reportBookingSyncFailure() - attempt an ops alert without blocking recovery.
// Delivery is not guaranteed when the network is unavailable. Uses the existing
// Worker; no new backend surface at all.
async function reportNadiSyncFailure(ref, payload, errorDetail) {
  try {
    await bookingRequest(`${NADI_API_BASE}/escalate`, {
      ...
      body: JSON.stringify({
        ...
        context: `POST /bookings failed for nadiairporttransfers.com guest booking ${ref}. Payload: ${JSON.stringify(payload)}. Error: ${JSON.stringify(errorDetail)}`.slice(0, 2000),
      }),
    }, 5000);
  } catch (err) {
    // Last line of defence - nothing more to do client-side if even the
...
async function confirmBooking() {
  ...
  let ref = state.pendingBookingAttempt?.fingerprint === attemptFingerprint
    ? state.pendingBookingAttempt.ref : null;
  try {
  ...
  state.pendingBookingAttempt = { ref, fingerprint: attemptFingerprint };
  ...
  // conversation channel regardless of which path below runs.
  const waUrl = buildWhatsAppURL(ref);

  // CEO P0 revenue recovery (2026-09-07, preview only) - attempt a real
  // server-side save for the case this build can safely support (see the
  ...
  // Show the result card; unknown saves offer an in-place, field-preserving retry.
  ...
  if (retry) retry.hidden = !saveAttempted || saveResult.ok;
  // A confirmed save remains locked until the explicit new-booking reset.
  // Unknown saves unlock only so the same request can be retried.
  state.confirmBookingInFlight = !!saveResult.ok;
  if (confirmBtn) {
    confirmBtn.disabled = !!saveResult.ok;
