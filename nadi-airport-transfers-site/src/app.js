...
async function submitNadiBooking(ref, destZone) {
  ...
  try {
    ...
    if (!res.ok || !data?.ok) {
      await reportNadiSyncFailure(ref, payload, data);
    ...
    await reportNadiSyncFailure(ref, payload, { error: err.message });
    return { ok: false, error: err.message };
  }
}

// Same non-blocking, fire-and-forget escalation pattern as FijiDash's
// reportBookingSyncFailure() - a failed save here still gets a human alert,
// never silently lost. Reuses the SAME /escalate endpoint on the SAME
...
async function confirmBooking() {
  ...
  // Pull customer first name for personalised greeting - still needed for
  // the WhatsApp-only fallback headline, which keeps its original wording.
  ...
  if (bulaWaBtn) {
    ...
    if (saveResult.ok) {
      ...
      if (lastChild && lastChild.nodeType === Node.TEXT_NODE) {
        lastChild.textContent = 'Send your reservation details on WhatsApp →';
      }
    }
